...
input {
  File full_map  #col1: sample name, col2: path to gvcf, col3: path to gvcf index /!\ only for  new-samples to be added
  ...
  Int sample_num_threshold = 50 #minimum number of samples to get reliable joint calling results
    ...
    Array[File] workspace_db ##array of tar'ed db, needing *independant names* for each db
    String callset_name
  ...
  scatter (idx in range(length(workspace_db))) {

    call updateGenomicsDB {
      input:
        GATK = GATK,
        sample_name_map = sample_name_map,
        workspace_db = workspace_db[idx],
        output_interval_file_name = idx + ".interval_list", #extension is important not have the task fail
        TMP_DIR='',
        WORKSPACE='',
        index = idx,
        batch_size = 50
    ...
    call Tasks.GenotypeGVCFs {
        input:
          GATK = GATK,
          workspace_tar = updateGenomicsDB.output_genomicsdb,
          interval = updateGenomicsDB.interval,
...
##                                    2- use final vcf as 2nd input instead of intermediate recalibrated vcfs
  ...
  output {
    ...
    Array[File] genomics_database = updateGenomicsDB.output_genomicsdb
    Array[File] output_intervals = updateGenomicsDB.interval
...
task updateGenomicsDB {
  input {
    File GATK
    File sample_name_map #sample_map is the map of  new-samples to be added
    File workspace_db
    String TMP_DIR
    String WORKSPACE
  ...
  command <<<
    ...
    #need untar'd db
    tar -xf ~{workspace_db}
    WORKSPACE=$(basename ~{workspace_db} .tar)
    ...
    tar -cf ${WORKSPACE}.~{index}.tar ${WORKSPACE}
  >>>
  ...
  output {
    File output_genomicsdb = "~{WORKSPACE}.~{index}.tar "
