...
if [ $# -eq 2 ]; then
 full_map=$1  #col1: sample ID, col2: path to cram, col3: path to cram index
 cohort_name=$2

else
 echo -e "\n\nLaunching runDellt script by sample\n\nAuthor: Claire Redin (claire.redin@chuv.ch)\n\n"
 echo "Usage:"
 echo "launch-Delly.sh [full_map] [cohort_name] "
 echo "full_map: full path to file with 3 tab-delimited columns. Col 1: sample ID, col 2: path to bam/cram, col 3: path to bam/cram index"
