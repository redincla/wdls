...
command <<<
  ...
  python /scratch/beegfs/PRTNR/CHUV/MED/jfellay/default_sensitive/redin/tools/TRAPD/code/count_cases.py --vcffile ~{input_vcf} --snpfile ~{variant_list} --outfile ~{base_output_name} --pass ~{sample_list_arg} --bedfile ~{regions_list}
...
command <<<
  ...
  Rscript /scratch/beegfs/PRTNR/CHUV/MED/jfellay/default_sensitive/redin/tools/TRAPD/code/burden.R --casefile ~{cases_count_file} --casesize ~{cases_size} --controlfile ~{ctrls_count_file} --controlsize ~{ctrls_size} --outfile ~{base_output_name}
>>>
